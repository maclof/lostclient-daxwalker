...
import org.lostclient.api.wrappers.walking.dax_api.api_lib.utils.DaxTrackerProperty;
...
public class DaxTracker implements Loggable {
     ...
     * @param propertyName
     * @param period
     * @return Accounts online belonging to the tracking user
     */
    public static ListSearch accountsOnline(String propertyName, Period period) {
        return DaxTrackerServerApi.getInstance().sourcesOnline(propertyName, DaxTracker.getTrackingUsername(), period);
    }
     ...
     * @param accountName
     * @param propertyName
     * @return Total accumulated stats for the tracking user.
    ...
    public static PropertyStats getStats(String accountName, String propertyName) {
        return DaxTrackerServerApi.getInstance().getStats(DaxTracker.getTrackingUsername(), accountName, propertyName);
    }
    ...
    public static SourceHighScore getAccountsHighScore(String propertyName, Period period) {
        return DaxTrackerServerApi.getInstance().topSources(DaxTracker.getTrackingUsername(), propertyName, period);
    }
    ...
    public boolean update() {
        if (MethodProvider.timeFromMark(lastUpdated) < 10000) {
            return false;
        }
    ...
    private static void log(String propertyName, double value) {
        ...
        DaxTrackerServerApi.getInstance().log(
                DaxTracker.getTrackingUsername(),
                accountName,
                propertyName,
                value
        );

...
public enum TeleportScrolls implements Validatable {
	...
	public boolean teleportTo(boolean shouldWait){
		Item scroll = Inventory.get(this.name);
		return scroll.interact(scroll.getActions()[0]) && (!shouldWait || MethodProvider.sleepUntil(() -> this.location.distance(Players.localPlayer().getTile()) < 15, 8000));
	}

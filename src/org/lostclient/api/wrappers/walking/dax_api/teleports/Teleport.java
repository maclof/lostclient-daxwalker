...
import org.lostclient.api.wrappers.walking.dax_api.api_lib.models.Requirement;
...
import org.lostclient.api.wrappers.walking.dax_api.shared.helpers.magic.Spell;
import org.lostclient.api.wrappers.walking.dax_api.teleports.teleport_utils.TeleportConstants;
import org.lostclient.api.wrappers.walking.dax_api.teleports.teleport_utils.TeleportLimit;
import org.lostclient.api.wrappers.walking.dax_api.teleports.teleport_utils.TeleportScrolls;
...
public enum Teleport {
	...
	VARROCK_TELEPORT_TAB(
		35, new RSTile(3212, 3424, 0),
		() -> inMembersWorld() && Inventory.count("Varrock teleport") > 0,
	...
	LUMBRIDGE_TELEPORT_TAB(
		35, new RSTile(3225, 3219, 0),
		() -> inMembersWorld() && Inventory.count("Lumbridge teleport") > 0,
	...
	FALADOR_TELEPORT_TAB(
		35, new RSTile(2966, 3379, 0),
		() -> inMembersWorld() && Inventory.count("Falador teleport") > 0,
	...
	CAMELOT_TELEPORT_TAB(
		35, new RSTile(2757, 3479, 0),
		() -> inMembersWorld() && Inventory.count("Camelot teleport") > 0,
		() -> ItemHelper.click("Camelot t.*", "Break")
	),

	SEERS_TELEPORT(
		35, new RSTile(2757, 3479, 0),
		() -> inMembersWorld() && Spell.CAMELOT_TELEPORT.canUse() && PlayerSettings.getBitValue(4560) == 1,
	...
	ARDOUGNE_TELEPORT_TAB(
		35, new RSTile(2661, 3300, 0),
		() -> inMembersWorld() && Inventory.count("Ardougne teleport") > 0,
	...
	RING_OF_WEALTH_MISCELLANIA(
		...
		() -> inMembersWorld() && WearableItemTeleport.has(WearableItemTeleport.RING_OF_WEALTH_FILTER) && PlayerSettings.getConfig(359) >= 100,
	...
	WEST_ARDOUGNE_TELEPORT_TAB(
		...
		() -> inMembersWorld() && Inventory.count("West ardougne teleport") > 0,
	...
	SALVE_GRAVEYARD_TAB(
		...
		() -> inMembersWorld() && Inventory.count("Salve graveyard teleport") > 0,
	...
	public boolean isAtTeleportSpot(RSTile tile) {
		return tile.distance(location) < 10;
	}
	...
	public static List<RSTile> getValidStartingTiles() {
	...
	private static boolean inMembersWorld() {
		...
		value = Worlds.getMyWorld().isMember();
		return value;
	}

	private static Predicate<Item> notNotedFilter() {
		return rsItem -> rsItem.isNoted();
	}

	private static boolean itemAction(String name, String... actions) {
		Item item = Inventory.get(name);
		return item != null && item.interact(actions[0]);
	}
	...
	private static boolean teleportWithScrollInterface(Predicate<Item> itemFilter, String regex){
		...
		if(Widgets.getWidgetChild(TeleportConstants.SCROLL_INTERFACE_MASTER) == null){
			Item teleportItem = items.get(0);
			if (!ItemHelper.clickMatch(teleportItem, "(Rub|Teleport|" + regex + ")") ||
				!MethodProvider.sleepUntil(() -> InterfaceHelper.isInterfaceSubstantiated(
	...
	private static boolean handleScrollInterface(String regex){
		WidgetChild box = Widgets.getWidgetChild(187, 3);
		if(box == null)
			return false;
		List<WidgetChild> children = box.getChildren();
		if(children == null)
			return false;
		for(WidgetChild child:children){
			String txt = child.getText();
			if(txt != null && StringHelper.stripFormatting(txt).matches(regex)){
				Keyboard.type(StringHelper.stripFormatting(txt).substring(0,1), true);
				return true;
			}
		}
		return false;
	...
	private static boolean selectSpell(String spellName, String action){
		...
		List<WidgetChild> spells = InterfaceHelper.getAllInterfaces(TeleportConstants.SPELLBOOK_INTERFACE_MASTER);
		WidgetChild target = spells.stream().filter(spell -> {
			String name = spell.getName();
			return name != null && name.contains(spellName) && !spell.isHidden();
		}).findFirst().orElse(null);
		return target != null && target.interact(action);
	}

	private static boolean hasBeenToZeah(){
		return PlayerSettings.getBitValue(4897) > 0;
	}
}
